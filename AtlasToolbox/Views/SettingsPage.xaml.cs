...
namespace AtlasToolbox.Views
{
    ...
    {
        public bool KeepBackground_State = RegistryHelper.IsMatch("HKLM\\SOFTWARE\\PCTools\\Toolbox", "KeepInBackground", 1);
        ...
        private void IsExperimentEnabled(object sender, RoutedEventArgs e)
        {
            ...
            s.IsOn = RegistryHelper.IsMatch(@$"HKLM\SOFTWARE\PCTools\Toolbox\Experiments\{s.Tag.ToString()}", "enabled", 1);
        ...
        {
            var s = sender as ToggleSwitch;
            RegistryHelper.SetValue(@$"HKLM\SOFTWARE\PCTools\Toolbox\Experiments\{s.Tag.ToString()}", "enabled", s.IsOn, Microsoft.Win32.RegistryValueKind.DWord);
            App.ContentDialogCaller("restartApp");
        }
