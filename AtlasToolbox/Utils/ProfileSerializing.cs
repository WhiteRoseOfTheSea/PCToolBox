...
namespace AtlasToolbox.Utils
{
    public static class ProfileSerializing
    {
        ...
        {
            FileInfo[] profileFile = new DirectoryInfo($"{Environment.GetEnvironmentVariable("windir")}\\PCToolsModules\\Toolbox\\Profiles\\").GetFiles();
            ...
            File.WriteAllText($"{Environment.GetEnvironmentVariable("windir")}\\PCToolsModules\\Toolbox\\Profiles\\{profileName}.json", jsonString);
            return DeserializeProfile($"{Environment.GetEnvironmentVariable("windir")}\\PCToolsModules\\Toolbox\\Profiles\\{profileName}.json");
        }
